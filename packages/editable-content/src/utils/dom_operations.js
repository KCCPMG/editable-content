...
exports.deleteEmptyElementsByQuery = deleteEmptyElementsByQuery;
exports.deleteEmptyElements = deleteEmptyElements;
exports.unwrapSelectionFromQuery = unwrapSelectionFromQuery;
exports.unwrapRangeFromQuery = unwrapRangeFromQuery;
exports.promoteChildrenOfNode = promoteChildrenOfNode;
exports.generateQuery = generateQuery;
exports.createWrapper = createWrapper;
exports.cushionTextNode = cushionTextNode;
exports.resetTextNodesCushions = resetTextNodesCushions;
exports.clearAndResetSelection = clearAndResetSelection;
 ...
 * @returns
 */
function generateQueryFromElement(element) {
 ...
 * @returns
 */
function wrapInElement(selection, element, limitingContainer) {
    var _a, _b, _c;
    if (!selection)
        return;
    const unbreakables = [];
    ...
    if (range.startContainer && range.startContainer.nodeType === Node.TEXT_NODE) {
        if (range.startContainer.textContent === "") {
            (_a = range.startContainer.parentNode) === null || _a === void 0 ? void 0 : _a.removeChild(range.startContainer);
        }
    }
    if (range.endContainer && range.endContainer.nodeType === Node.TEXT_NODE) {
        if (range.endContainer.textContent === "") {
            (_b = range.endContainer.parentNode) === null || _b === void 0 ? void 0 : _b.removeChild(range.endContainer);
        }
    }
    ...
    for (let unbreakable of unbreakables) {
        ...
        if (unbreakableRange.endContainer.nodeType !== Node.TEXT_NODE) {
            ...
            unbreakableRange.setEnd(lastTextNode, ((_c = lastTextNode.textContent) === null || _c === void 0 ? void 0 : _c.length) || 0);
        }
        unwrapRangeFromQuery(unbreakableRange, query, limitingContainer);
    }
    (0, selection_movements_1.resetSelectionToTextNodes)(); // make sure we're in text nodes
 ...
 * @returns
 */
function deleteEmptyElementsByQuery(query, limitingContainer) {
 ...
 * @param limitingContainer
 */
function deleteEmptyElements(limitingContainer) {
 ...
 * @returns
 */
function unwrapSelectionFromQuery(selection, query, limitingContainer) {
 ...
 * @returns
 */
function unwrapRangeFromQuery(range, query, limitingContainer) {
    ...
    const preAncestorNode = (0, checks_1.getAncestorNode)(range.startContainer, query, limitingContainer); // assumes startContainer is text node
    if (!preAncestorNode)
        return;
    ...
    const postAncestorNode = (0, checks_1.getAncestorNode)(range.endContainer, query, limitingContainer); // assumes endContainer is text node
    if (!postAncestorNode)
        return;
    ...
    const startContainerAncestorNode = (0, checks_1.getAncestorNode)(range.startContainer, query, limitingContainer);
    if (startContainerAncestorNode) {
        range.setStartBefore(startContainerAncestorNode);
    }
    const endContainerAncestorNode = (0, checks_1.getAncestorNode)(range.endContainer, query, limitingContainer);
    ...
    const childNodes = (0, checks_1.getRangeChildNodes)(range, limitingContainer);
 ...
 * @returns
 */
function generateQuery({ element, id, unbreakable, attributes }) {
 ...
 * @returns
 */
function createWrapper({ element, classList, id, unbreakable, attributes, eventListeners }, document) {
...
function cushionTextNode(textNode) {
    if (!(textNode instanceof Text) || textNode.textContent === null)
        return;
...
function resetTextNodesCushions(textNodes) {
    textNodes.forEach(tn => {
        if (!(0, checks_1.textNodeIsCushioned)(tn))
            cushionTextNode(tn);
    });
}
/**
 ...
 * @returns
 */
function clearAndResetSelection(selection) {
    const range = selection.getRangeAt(0);
