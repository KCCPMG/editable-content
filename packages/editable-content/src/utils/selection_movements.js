...
exports.resetSelectionToTextNodes = resetSelectionToTextNodes;
exports.resetRangeToTextNodes = resetRangeToTextNodes;
exports.resetSelectionToUsableText = resetSelectionToUsableText;
exports.moveSelection = moveSelection;
exports.extendSelection = extendSelection;
exports.extendWordSelection = extendWordSelection;
 ...
 * @returns
 */
function resetSelectionToTextNodes() {
 ...
 * @returns
 */
function resetRangeToTextNodes(range) {
    var _a;
    ...
    if (range.endContainer.nodeType !== Node.TEXT_NODE) {
        ...
        range.setEnd(lastTextNode, ((_a = lastTextNode.textContent) === null || _a === void 0 ? void 0 : _a.length) || 0);
    }
    return range;
}
/**
 ...
 * @returns
 */
function resetSelectionToUsableText() {
    let selection = window.getSelection();
    if (!selection)
        return selection;
    const range = selection.getRangeAt(0);
    ...
    if (!((0, checks_1.textNodeIsCushioned)(range.startContainer))) {
        (0, dom_operations_1.cushionTextNode)(range.startContainer);
    }
    if (!((0, checks_1.textNodeIsCushioned)(range.endContainer))) {
        (0, dom_operations_1.cushionTextNode)(range.endContainer);
    }
    /**
 ...
 * @returns
 */
function moveSelection(selection, limitingContainer, moveDirection) {
    ...
    if (direction === "none") {
        if (moveDirection === "left") {
            if (!(range.startContainer instanceof Text))
                return;
            const nextPosition = (0, checks_1.getNextPosition)(range.startContainer, range.startOffset, limitingContainer, "left", "[^\u200B]", false, 0, true, true);
            ...
            if (!(range.startContainer instanceof Text))
                return;
            const nextPosition = (0, checks_1.getNextPosition)(range.startContainer, range.startOffset - 1, limitingContainer, "right", "[^\u200B]", true, 0, true, true);
            if (nextPosition === null)
                return;
            // else
            return selection.setBaseAndExtent(nextPosition.currentNode, nextPosition.offset, nextPosition.currentNode, nextPosition.offset);
 ...
 * @returns
 */
function extendSelection(selection, limitingContainer, moveDirection) {
    const { anchorNode, anchorOffset, focusNode, focusOffset } = selection;
    const range = selection.getRangeAt(0);
    if (!range ||
        !anchorNode ||
    ...
    if (moveDirection === "left") {
        if (!(range.startContainer instanceof Text))
            return;
        const nextPosition = (0, checks_1.getNextPosition)(focusNode, focusOffset, limitingContainer, "left", "[^\u200B]", false, 0, false, true);
        if (nextPosition === null)
            return;
        // else
        ...
        if (!(range.startContainer instanceof Text))
            return;
        const nextPosition = (0, checks_1.getNextPosition)(focusNode, focusOffset - 1, limitingContainer, "right", "[^\u200B]", true, 0, false, true);
        if (nextPosition === null)
            return;
        // else
        return selection.setBaseAndExtent(anchorNode, anchorOffset, nextPosition.currentNode, nextPosition.offset);
 ...
 * @returns
 */
function extendWordSelection(selection, limitingContainer, moveDirection) {
    const { anchorNode, anchorOffset, focusNode, focusOffset } = selection;
    const range = selection.getRangeAt(0);
    if (!range ||
        !anchorNode ||
    ...
    if (moveDirection === "left") {
        if (!(range.startContainer instanceof Text))
            return;
        const nextPosition = (0, checks_1.getNextPosition)(focusNode, focusOffset - 1, limitingContainer, "left", "\\u200B| \\S", false, 1, false, true);
        if (nextPosition === null)
            return;
        // else
        return selection.setBaseAndExtent(anchorNode, anchorOffset, nextPosition.currentNode, nextPosition.offset);
        ...
        if (!(range.startContainer instanceof Text))
            return;
        const nextPosition = (0, checks_1.getNextPosition)(focusNode, focusOffset, limitingContainer, "right", "\\S |\\S\\u200B$", false, 1, false, true);
        if (nextPosition === null)
            return;
        // else
        return selection.setBaseAndExtent(anchorNode, anchorOffset, nextPosition.currentNode, nextPosition.offset);
