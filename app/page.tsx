...
export default function Page() {
  ...
  return (
    <>
      ...
      <p>
        Welcome to the <strong>editable-content</strong> demo! This library is designed to give developers the ability to create rich-text editors which include React Components as elements.
      </p>
      <p>
        As a demonstration, please feel free to play around with the editable text container below. It should work like a normal rich text field, but with one key distinction: there are two types of styling buttons: <strong>Standard Wrappers</strong> and <strong>Unbreakable Components.</strong>
      </p>
      <p>
        <strong>Standard Wrappers</strong> work as you would normally expect from a word processor; when text is selected and the button is clicked, the selected text will take on that text style. Click the button again, and the selected text will lose that given text decoration. Text can have multiple standard wrappers in effect at the same time, such as text which is in italics and in bold while inside of other text which is bold. When the cursor is in just one space, clicking the button will make future text from the cursor have that decoration, or lose that style if the button was already clicked.
      </p>
      <p>
